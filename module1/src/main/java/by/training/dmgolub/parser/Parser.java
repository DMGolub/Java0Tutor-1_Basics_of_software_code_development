...
public class Parser {
    ...
    /**
     * Parses BigInteger value from Scanner. If the specified input is not a BigInteger value,
     * asks user to retry input.
     * @param scanner Scanner,
     * @param variableName String variable name.
     * @return BigInteger value parsed from input.
     */
    public static BigInteger tryParseBigInteger(Scanner scanner, String variableName) {
        if (scanner == null) {
            throw new IllegalArgumentException("Scanner can not be null!");
        }
        if (variableName == null) {
